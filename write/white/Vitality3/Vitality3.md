...
### 1.3 Mechanizing Play as Vitality

Play has the right structure — but structure alone is not a theory. To go from the observation that play exhibits bidirectional engagement and symbolic abstraction to a mechanistic account, we must identify the two capacities precisely, show how they close into a developmental loop, and specify the architecture that results. Section 1 does this informally; Section 2 does it formally. The parallel is deliberate: mechanizing play (here) and formalizing play (there) are two passes over the same structure at different levels of precision.

### 1.4 First Capacity: Dynamic Bidirectional Engagement
...
### 1.8 Klopf's Critique and the Nature of Autonomy
