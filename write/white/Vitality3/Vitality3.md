...
**Second duality: Dual-laws model.** The system operates at two levels simultaneously, and feedback error is independently reduced by both. This is the dual-laws architecture (Ohmura et al.): the supervenience level and the base level constitute two distinct control mechanisms operating on the same error signal.

At the **supervenience level** (symbolic/index layer), the system is composed of multiple supervenient functions. Dynamical equations are constructed by combining these functions according to an *index sequence* — a discrete ordering or selection that determines which equations are active. The dynamical laws at this level modify the index sequence itself. This is **Type 2 processing**: discrete, exploratory, adjusting *which* equations govern the system by reordering or selecting index sequences. This is the symbolic abstraction capacity — operating on the structural combinatorics of the system's equations rather than on their continuous solutions.

At the **base level** (neuronal/dynamics layer), we find neurons, synapses, and other components that constitute the supervenient functions themselves. These adjust continuously to satisfy whichever equations are currently specified by the supervenience level. This is **Type 1 processing**: continuous, goal-directed, inter-level feedback control that modifies the functions themselves to reduce error under the constraints imposed from above.

The key insight is that feedback error can be reduced in two independent ways: by changing the index sequence at the supervenience level (Type 2, discrete) or by adjusting neural states at the base level (Type 1, continuous). Both mechanisms operate on the same error signal but through orthogonal degrees of freedom. This is what makes the dual-laws model work — the system can explore the space of *which equations to use* separately from exploring *how to satisfy those equations*.

This maps directly onto dual process theory. Type 1 is the continuous adjustment of functions at the base level — fast, automatic, subsymbolic. Type 2 is discrete control through index sequence modification at the supervenience level — slow, deliberate, symbolic. The dual-dual architecture is thus: (1) E–P dialectic governing the bidirectional engagement dynamics, and (2) dual-laws Type 1/Type 2 structure separating symbolic recombination from neuronal adjustment.

Both methods of control are naturally incorporated in the dual-laws model, and there is no reason to believe Type 2 exists only in humans. However, since Type 2 is inherently discrete — operating on index sequences rather than continuous dynamics — it is plausible that it becomes particularly advanced in organisms with sophisticated linguistic capacities, humans being the clearest case. Language itself is an index system, and the capacity for discrete symbolic recombination that enables language may be the same capacity that drives Type 2 processing.

**The triadic observation.** This dual-laws structure gives rise to a natural three-layer phenomenology. At the top sits the E–P intrinsic motivation itself — the *super-conscious* layer, the pre-symbolic drive toward expanded bidirectional engagement that selects what is worth attending to. In the middle is the symbolic index-sequence layer — the *conscious* layer of discrete, addressable, manipulable abstractions where Type 2 processing operates. At the bottom is the neuronal dynamics substrate — the *sub-conscious* layer where Type 1 processing continuously adjusts the functions that implement the symbolic layer's specifications. The triad (super-conscious IM / conscious symbolic / sub-conscious neuronal) is a consequence of the dual-laws architecture, not a separate structural principle. The formal structure is the dual-laws model; the triadic mapping is an observation about what that structure looks like from the inside.
